...
<div class="content-wrapper">
    ...
    <div id="search-section" class="chart-card" style="display:none;">
        <div class="section-header">
            <div class="section-title">Matrix Diagnostic</div>
    ...
    <div id="report-section" style="display:none; flex-direction:column; gap:40px; width:100%;">
        ...
        <div class="section-block">
            <h3
                style="margin-left:5px; margin-bottom:10px; color:#ddd; border-left: 4px solid #1E90FF; padding-left:10px;">
                Topic Votes</h3>
        ...
        <div class="section-block">
            <h3
                style="margin-left:5px; margin-bottom:10px; color:#ddd; border-left: 4px solid #d946ef; padding-left:10px;">
                Post Votes</h3>
        ...
        fetch('/shadowpulse/api/v1/vote_pyramid.php?uuid=' + uuid)
            ...
            .then(data => {
                if (data.success) {
                    ...
                    renderGrid('#topic-queue', data.topic_queue, 'topic');

                    // Render Post Data (Pink/Purple Theme)
                    renderChart('#post-chart', data.post_pyramid, '#d946ef');
                    renderGrid('#post-queue', data.post_queue, 'post');
                } else {
            ...
            columns: [
                {
                    ...
                    template: (props) => {
                        let url = '#';
                        if (type === 'topic') {
                            url = `https://bitcointalk.org/index.php?topic=${props.target_id}.0`;
                        } else if (type === 'post') {
                            url = `https://bitcointalk.org/index.php?msg=${props.target_id}`;
                        }
                        return `<a href="${url}" target="_blank" style="color:#4ade80; text-decoration:none;">${props.target_id}</a>`;
                    }
                },
...
<div style="font-size: 10px; color: #666; text-align: center; margin-top: 20px;">Matrix Report v1 (v0.35.1)</div>
