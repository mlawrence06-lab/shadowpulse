-- Create table for tracking custom pages
CREATE TABLE IF NOT EXISTS stats_custom_pages (
    ...
    UNIQUE KEY idx_url_hash (page_url_hash)
...
BEGIN
    ...
    IF (v_member_id = 0 OR v_member_id IS NULL) AND p_uuid != '' THEN
        INSERT INTO members (member_uuid) VALUES (p_uuid);
        SET v_member_id = LAST_INSERT_ID();
        -- INSERT IGNORE INTO member_stats (member_id) VALUES (v_member_id); -- Moved below
    END IF;

    -- Ensure stats row exists for EVERYONE (Self-Healing)
    INSERT IGNORE INTO member_stats (member_id) VALUES (v_member_id);

    -- Track Page View (Restored)
    UPDATE member_stats 
    SET page_views = page_views + 1, last_active = CURRENT_TIMESTAMP 
    WHERE member_id = v_member_id;

    -- Update Metadata (Profile Name) and View Count
    IF p_category = 'profile' AND p_target_id > 0 THEN
        INSERT INTO stats_profiles (member_id, profile_name, view_count) VALUES (p_target_id, p_title, 1)
        ON DUPLICATE KEY UPDATE 
            view_count = view_count + 1,
            profile_name = CASE WHEN p_title IS NOT NULL AND p_title != '' THEN p_title ELSE profile_name END;
    END IF;

    -- Track Topic Views
    IF p_category = 'topic' AND p_target_id > 0 THEN
        INSERT INTO stats_topics (topic_id, view_count) VALUES (p_target_id, 1)
        ON DUPLICATE KEY UPDATE view_count = view_count + 1;
        -- Note: stats_topics usually created on vote? If auto-created here, defaults score 0. Correct.
    END IF;

    -- Track Post Views
    IF p_category = 'post' AND p_target_id > 0 THEN
         INSERT INTO stats_posts (post_id, view_count) VALUES (p_target_id, 1)
         ON DUPLICATE KEY UPDATE view_count = view_count + 1;
    END IF;
    
    -- Track Board Views
    IF p_category = 'board' AND p_target_id > 0 THEN
         INSERT INTO stats_boards (board_id, view_count) VALUES (p_target_id, 1)
         ON DUPLICATE KEY UPDATE view_count = view_count + 1;
    END IF;

    -- Track Custom Pages (Special Category)
    IF p_category = 'special' AND p_url IS NOT NULL AND p_url != '' THEN
        INSERT INTO stats_custom_pages (page_url_hash, page_url, page_title, view_count) 
        VALUES (SHA2(p_url, 256), p_url, p_title, 1)
        ON DUPLICATE KEY UPDATE 
            view_count = view_count + 1,
            page_title = CASE WHEN p_title IS NOT NULL AND p_title != '' THEN p_title ELSE page_title END;
    END IF;
