<?php
// migrate_page_views.php
// Adds page_views columns and updates Stored Procedure
...
try {
    ...
    // 1. Add Columns (Try/Catch ignore duplicate)
    // Use raw ALTER TABLE without IF NOT EXISTS
    $tables = [
        'stats_topics' => 'ALTER TABLE stats_topics ADD COLUMN page_views BIGINT UNSIGNED DEFAULT 0',
        'stats_posts' => 'ALTER TABLE stats_posts ADD COLUMN page_views BIGINT UNSIGNED DEFAULT 0',
        'stats_profiles' => 'ALTER TABLE stats_profiles ADD COLUMN page_views BIGINT UNSIGNED DEFAULT 0',
    ];
    ...
    // 2. Create/Update stats_boards
    // Check if exists first (or just Create If Not Exists)
    $sqlBoard = "CREATE TABLE IF NOT EXISTS stats_boards (
    ...
    $pdo->exec($sqlBoard);
    echo "Ensured stats_boards exists.\n";

    // Add page_views if table already existed without it
    try {
        $pdo->exec("ALTER TABLE stats_boards ADD COLUMN IF NOT EXISTS page_views BIGINT UNSIGNED DEFAULT 0");
    } catch (Exception $e) { /* ignore */
    }


    // 3. Drop and Recreate Stored Procedure
    $spSQL = <<<SQL
...
BEGIN
    ...
    SELECT 
        ...
        COALESCE(
            ...
            spr.profile_name,
            -- Need board name logic or just return ID if local lookup
            NULL 
        ) as target_label,
