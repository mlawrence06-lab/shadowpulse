...
echo "<h1>Starting Hardcoded Fix</h1>";

// 1. Definte Update Procedure
$sql1 = "
DROP PROCEDURE IF EXISTS `shadowpulse_update_member_stats`;
CREATE PROCEDURE `shadowpulse_update_member_stats`(IN p_member_id BIGINT)
BEGIN
    DECLARE v_topic_count INT DEFAULT 0;
    DECLARE v_post_count INT DEFAULT 0;
    SELECT COUNT(*) INTO v_topic_count FROM votes WHERE member_id = p_member_id AND vote_category = 'topic';
    SELECT COUNT(*) INTO v_post_count FROM votes WHERE member_id = p_member_id AND vote_category = 'post';
    INSERT INTO member_stats (member_id, topic_votes, post_votes)
    VALUES (p_member_id, v_topic_count, v_post_count)
    ON DUPLICATE KEY UPDATE
        topic_votes = v_topic_count,
        post_votes = v_post_count;
END;
";

// 2. Define Cast Vote Procedure
$sql2 = "
...
BEGIN
  ...
  promote_L2_to_L3: LOOP
    ...
    -- SYNC: Update the aggregate stats for this member
    CALL shadowpulse_update_member_stats(p_member_id);
...
try {
    $pdo->exec($sql1);
    echo "<p>Procedure 1 Created.</p>";
    $pdo->exec($sql2);
    echo "<p>Procedure 2 Created.</p>";

    $pdo->exec("CALL shadowpulse_update_member_stats(752)");
    echo "<p>Stats Backfilled for 752.</p>";

    // Check Result
    $stmt = $pdo->query("SELECT * FROM member_stats WHERE member_id = 752");
    $row = $stmt->fetch(PDO::FETCH_ASSOC);
    echo "<p>FINAL STATS for 752: Topic Votes = " . $row['topic_votes'] . ", Post Votes = " . $row['post_votes'] . "</p>";
