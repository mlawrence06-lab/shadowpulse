...
// Error Handling: Suppress output, log to file only
...
// Clean Title for Profile and Board
if ($title) {
    // Universal Clean: Always remove BTT Suffix if present
    $title = str_replace(' - Bitcoin Forum', '', $title);

    // Profile Specific: "View the profile of Username" -> "Username"
    if ($category === 'profile') {
        if (strpos($title, 'View the profile of ') !== false) {
            $start = strpos($title, 'View the profile of ') + strlen('View the profile of ');
...
try {
    ...
    // FIX: Recalculate Rank if SP returns suspicious value (e.g. 1 when score < 5) OR always recalculate to be safe.
    // Since SP update failed due to permissions, we force recalculation here.
    if (!empty($category) && !empty($targetId)) {
        $currentScore = (float) ($context['average_score'] ?? 0);
        $currentRank = (int) ($context['item_rank'] ?? 0);

        // Only recalc for valid categories
        $tableName = '';
        $idCol = '';
        if ($category === 'topic') {
            $tableName = 'stats_topics';
            $idCol = 'topic_id';
        }
        if ($category === 'post') {
            $tableName = 'stats_posts';
            $idCol = 'post_id';
        }
        if ($category === 'profile') {
            $tableName = 'stats_profiles';
            $idCol = 'member_id';
        }

        if ($tableName) {
            // Count items with strictly higher score OR same score but created earlier (lower ID)
            $rankSql = "SELECT COUNT(*) + 1 as real_rank FROM $tableName WHERE average_score > ? OR (average_score = ? AND $idCol < ?)";
            $rStmt = $pdo->prepare($rankSql);
            $rStmt->execute([$currentScore, $currentScore, $targetId]);
            $rRow = $rStmt->fetch(PDO::FETCH_ASSOC);
            if ($rRow) {
                $response['context']['summary']['rank'] = (int) $rRow['real_rank'];
            }
        }
    }
